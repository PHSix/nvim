...
function M.try(...)
  for _,vfn in ipairs({...}) do
    local ok, _ = pcall(vfn)
    if ok then
      return
    end
  end
...
function M.actions.buf_tags()
  ...
  assert(fn.filereadable(bufname), 'File to generate tags for must be readable')
  ...
  if not ok or api.nvim_get_vvar('shell_error') ~= 0 then
    output = fn.system({'ctags', '-f', '-', '--sort=yes', '--excmd=number', bufname})
  end
...
function M.actions.quickfix()
  ...
  M.pick_one(
    ...
    'Quickfix> ',
    function(item)
      local bufname = fn.fnamemodify(api.nvim_buf_get_name(item.bufnr), ':t')
      return bufname .. ': ' .. item.text
    end,
    function(item)
      if not item then return end
      fn.bufload(item.bufnr)
...
function M.pick_one(items, prompt, label_fn, cb)
  ...
  fn.system(string.format('mkfifo "%s"', inputs))
  M.execute(
