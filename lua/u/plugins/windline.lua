...
basic.lsp_diagnos = {
	...
	text = function(bufnr)
		...
		local diagnostic = vim.b.coc_diagnostic_info or {}
		local getMsg = function(attr, format)
			if diagnostic[attr] ~= nil then
				return string.format(format, diagnostic[attr])
			end
			return ""
		end
		if not vim.tbl_isempty(diagnostic) then
			return {
				-- { vim.g.coc_status or "", "gray" },
				{ getMsg('error', '  %s'), 'red' },
				{ getMsg('warning', '  %s'), 'yellow' },
				{ getMsg('hint', '  %s'), 'blue' },
			}
		end
		-- return {
		-- (vim.b.coc_diagnostic_info or {})
	...
	autocmd = { "User CocDiagnosticChange", "User CocStatusChange" }
}
